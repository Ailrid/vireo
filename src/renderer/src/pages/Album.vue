<template>
  <div class="h-full w-full flex-col">
    <div v-if="act.songs" class="flex h-full w-full">
      ...
      >
        ...
        <div class="flex h-full w-full flex-1 flex-col overflow-hidden pr-8">
          ...
          <div class="mb-6 aspect-square w-full shrink-0 overflow-hidden rounded-xl shadow-lg">
            <Img
              v-if="act.songs!.at(0)"
              :cover="act.songs!.at(0)!.album.cover"
          ...
          <div class="mb-6 flex w-full gap-2">
            <div class="flex-1"></div>
            <Button variant="icon" @click="act.setPlaylist(act.songs!.at(0)!)">
          ...
          <div class="flex flex-1 flex-col justify-start">
            ...
            <p class="line-clamp- mb-6 text-xs leading-relaxed opacity-60">
              {{ act.album?.description ? act.album?.description : '' }}
            </p>
          </div>
          <div class="flex-1"></div>
        </div>
          ...
          <div class="flex w-full items-end justify-between border-b border-current/10 pb-4">
            <div class="flex items-center">
              ...
              <p class="mr-2 ml-2 text-sm tracking-[0.2em] uppercase opacity-40">
                Total {{ act.songs?.length || 0 }} Tracks
              </p>
            ...
            <div class="font-mono text-xs opacity-40">SHARD: 0 - {{ act.songs?.length || 0 }}</div>
          </div>
          <!-- 列表 -->
          <div class="flex-1 overflow-y-auto pt-4">
            <div class="w-full">
              <div
                v-for="(item, index) in act.songs"
                :key="item.id"
...
import { AlbumPageController } from './controllers'
...
const act = useController(AlbumPageController)
</script>
