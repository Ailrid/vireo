<template>
  <div class="animation flex w-96 flex-col shadow-2xl">
    <!-- 背景图片 -->
    <div class="relative h-36 w-full overflow-hidden">
      <Img
        v-if="cct.userProfile?.avatar"
        :cover="cct.userProfile.avatar"
    ...
    <div class="relative w-full px-8 pb-8">
      ...
      <div class="pt-14">
        <div class="flex items-center gap-3">
          ...
          <span class="bg-primary rounded-md border px-2 py-0.5 text-xs font-black italic">
            Lv.{{ cct.userProfile?.level }}
          </span>
        </div>
        <p class="mt-3 line-clamp-2 min-h-[2.5rem] text-sm leading-relaxed">
    ...
    <div class="flex w-full items-center justify-between px-8 py-5">
      ...
      <Button variant="outline" @click="LogoutMessage.send()">
        <i class="ri-logout-box-r-line mr-1 text-base"> 退出登录</i>
