...
class RustEffectProcessor extends AudioWorkletProcessor {
  constructor(options) {
    ...
    if (wasmModule) {
      ...
      this.port.onmessage = event => {
        if (event.data.type == 'set_params') {
          const newParams = PipelineParams.new(
            ...
            event.data.params.crystallization_gain,
            event.data.params.stereo_gain,
            event.data.params.limiter_gain
          )
      ...
      this.inL = new Float32Array(heap, this.kernel.in_l_ptr(), 64)
      this.inR = new Float32Array(heap, this.kernel.in_r_ptr(), 64)
      this.outL = new Float32Array(heap, this.kernel.out_l_ptr(), 64)
      this.outR = new Float32Array(heap, this.kernel.out_r_ptr(), 64)
    } else {
  ...
  process(inputs, outputs) {
    ...
    for (let i = 0; i < 2; i++) {
      const offset = i * 64
      this.inL.set(input[0].subarray(offset, offset + 64), 0)
      if (input[1]) {
        this.inR.set(input[1].subarray(offset, offset + 64), 0)
      }
